# Pragmatic Play `/promoWin` Endpoint — Documentação Técnica

**Endpoint:** `POST /v1/webhooks/pragmatic-play/promoWin`  
**Provider:** Pragmatic Play  
**Funcionalidade:** Registrar pagamento de prêmio promocional do jogador  
**Status:** ✅ Documentação Fase 2  

> 🏗️ **Família handleResult() — 4º e último membro:** `/promoWin` encerra a família handleResult(). O wrapper público `promoWin()` (linhas ~109-112) delega para `handleResult('promoWin', data)` — a mesma lógica privada compartilhada com `/result`, `/bonusWin` e `/jackpotWin`. Referência canônica da família: `pragmatic-play-result.md`.

---

## 1. Resumo Executivo

O endpoint `/promoWin` registra o pagamento de um prêmio promocional vinculado a uma campanha específica do operador. É um evento distinto de bônus e jackpot: enquanto `/bonusWin` registra bônus do jogo e `/jackpotWin` registra jackpots acumulados, `/promoWin` é disparado por promoções definidas pelo operador (ex.: torneios, missões, cashback promocional). Tecnicamente, compartilha 100% da implementação PHP com os demais membros via `handleResult()`. A resposta é retornada **sem nenhuma modificação** (passthrough direto).
...
- 🏗️ **Arquitetura:** thin wrapper `promoWin()` → `handleResult('promoWin', data)`
- 🔚 **Encerra a família handleResult()** — 4 endpoints, 1 implementação compartilhada

**Fonte PHP:**
- Wrapper: `PragmaticPlayService.php` — método `promoWin()`, linhas ~109-112
...
```mermaid
graph TD
    A["<b>INPUT</b><br/>POST /v1/webhooks/pragmatic-play/promoWin<br/>{ userId, amount, currency, promoId, ... }"]

    A --> B["<b>FASE 1: ROTEAMENTO</b><br/>BR-GENERIC-ROUTING-VALIDATION-001<br/>BR-GENERIC-ERROR-HANDLING-001<br/>method_exists 'promoWin'<br/>✅ válido"]
    B --> B_err["❌ Endpoint inválido<br/>Exception 500"]
    B --> B2["promoWin() wrapper (~109-112)<br/>→ delega para handleResult('promoWin', data)"]
    ...
    G --> H["<b>FASE 7: HTTP POST</b><br/>BR-GENERIC-PROVIDER-INTEGRATION-001<br/>POST {operator.url}/pragmatic-play/promoWin.html<br/>Content-Type: application/json"]
...
|------|------|-------|-----------|
| 1 | Roteamento | BR-GENERIC-ROUTING-VALIDATION-001 + BR-GENERIC-ERROR-HANDLING-001 | `method_exists($service, 'promoWin')` → válido. Wrapper `promoWin()` delega imediatamente para `handleResult('promoWin', $data)`. |
...
| 7 | HTTP POST | BR-GENERIC-PROVIDER-INTEGRATION-001 | `postJson("{operator.url}/pragmatic-play/promoWin.html", payload)` — URL construída com argumento `'promoWin'`. |
...
**Entrada:**
```json
{ "amount": 20.00, "currency": "BRL", "promoId": "promo_tournament_001" }
```
...
**Entrada:**
```json
{ "userId": "semseparador", "amount": 20.00, "currency": "BRL" }
```
...
**Entrada:**
```json
{ "userId": "operadorinexistente_user123", "amount": 20.00, "currency": "BRL" }
```
...
### 4.6 Prêmio Promocional Rejeitado pelo Provider (`error != 0`)

**Provider responde:**
```json
{ "error": 4, "description": "Promotion expired or not applicable" }
```
...
**Cliente envia:**
```bash
curl -X POST http://localhost:8080/v1/webhooks/pragmatic-play/promoWin \
  ...
  -d '{
    ...
    "promoId": "promo_tournament_001",
    "transactionId": "txn_promo_001"
  }'
```
...
|------|----------|-------|--------|
| 1 | Routing + Delegação | endpoint="promoWin" | `promoWin()` wrapper → `handleResult('promoWin', data)` |
...
| 6 | Hash MD5 | sorted payload + secret | hash="f6a1b2c3d4e5..." |
| 7 | HTTP POST | `{url}/pragmatic-play/promoWin.html` | provider response recebida |
...
## 6. Contexto de Negócio: `/promoWin` vs Família handleResult()

| Aspecto | `/result` | `/bonusWin` | `/jackpotWin` | `/promoWin` |
|---------|-----------|------------|--------------|------------|
| **Evento** | Resultado de rodada | Bônus do jogo | Jackpot acumulado | **Prêmio de promoção do operador** |
| **Iniciador** | Provider | Provider | Provider | **Operador** (campanha configurada) |
| **Exemplos** | Rodada de slot | Free spins, bônus | Jackpot progressivo | Torneio, missão, cashback promocional |
| **Frequência** | Alta | Baixa | Muito baixa | Baixa (depende de campanhas ativas) |
| **Regras** | Contrato padrão | Regras de bônus | Regulatórias | **Definidas pelo operador por campanha** |
...
### Encerramento da Família handleResult()

Com `/promoWin`, a família handleResult() está **completa e documentada**:

```
handleResult() family — 4 endpoints, 1 lógica compartilhada:

  result()     → handleResult('result',     data) → .../result.html
  bonusWin()   → handleResult('bonusWin',   data) → .../bonusWin.html
  jackpotWin() → handleResult('jackpotWin', data) → .../jackpotWin.html
  promoWin()   → handleResult('promoWin',   data) → .../promoWin.html
```

O handler Go deve implementar `handleResult()` como função helper única invocada por 4 rotas independentes — replicando a mesma separação wrapper/lógica do PHP.
...
| Arquivo | Propósito |
|---------|-----------|
| `legacy/casino-proxy/app/Services/PragmaticPlayService.php:109-112` | Wrapper `promoWin()` |
